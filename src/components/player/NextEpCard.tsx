...
export function NextEpCard({ subtitle, countdown, countdownTotal, bottom, onPlay, onDismiss }: NextEpCardProps) {
  ...
  return (
    <div style={{ position: 'absolute', bottom, right: '1.375rem', zIndex: 4, minWidth: '13.75rem', maxWidth: '19.375rem' }}>
      <div
        style={{
      ...
      >
        <div style={{ padding: '0.625rem 0.75rem', display: 'flex', alignItems: 'flex-start', gap: '0.625rem' }}>
          <button
            onClick={(e) => { e.stopPropagation(); onPlay(); }}
            style={{ flex: 1, background: 'none', border: 'none', color: '#fff', cursor: 'pointer', textAlign: 'left', padding: 0, minWidth: 0 }}
          >
            <span style={{ fontSize: '0.625rem', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '0.1875rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {countdown !== null ? t('player.playing_in_seconds', countdown) : t('auto.up_next')}
            </span>
            {epCode && (
              <span style={{ fontSize: '0.6875rem', color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: '0.125rem' }}>{epCode}</span>
            )}
            <span style={{ fontSize: '0.8125rem', fontWeight: 700, display: 'block', lineHeight: 1.3 }}>{epTitle}</span>
          </button>
          <button
            onClick={(e) => { e.stopPropagation(); onDismiss(); }}
            style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', padding: '0.125rem 0 0 0', fontSize: '1rem', lineHeight: 1, flexShrink: 0, transition: 'color 0.15s' }}
        ...
        {progress !== null && (
          <div style={{ height: '0.125rem', background: 'rgba(255,255,255,0.10)' }}>
