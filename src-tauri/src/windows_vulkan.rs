...
fn load_proc(module: isize, name: &str) -> Result<*mut c_void, String> {
    ...
    addr.map(|f| f as *mut c_void)
        .ok_or_else(|| format!("vulkan-1.dll is missing {name}"))
}
...
impl VulkanContext {
    pub fn new(hwnd: isize, width: i32, height: i32) -> Result<Self, String> {
        ...
        let get_instance_proc_addr: PfnGetInstanceProcAddr =
            unsafe { std::mem::transmute(load_proc(module, "vkGetInstanceProcAddr")?) };
        ...
        let create_win32_surface_khr: PfnCreateWin32SurfaceKHR = iproc!("vkCreateWin32SurfaceKHR");
        ...
        let result = unsafe {
            create_win32_surface_khr(instance, &surface_create_info, ptr::null(), &mut surface)
        };
        if result != VK_SUCCESS {
        ...
        let fns = VkFns {
            get_instance_proc_addr,
            destroy_instance,
            get_physical_device_surface_support_khr: get_surface_support_khr,
